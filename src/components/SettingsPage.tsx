...
import type { PermissionState, Settings } from "../types";
import { RepositoryError } from "../types";

const formatShortcut = (value: string) => value
  .replace("Command", "⌘").replace("Control", "⌃").replace("Shift", "⇧").replace("Alt", "⌥")
  .split("+").join(" ");
...
export function SettingsPage({ repository }: { repository: ClipboardRepository }) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([repository.getSettings(), repository.getPermissionState()]).then(([nextSettings, nextPermission]) => {
      if (!active) return;
  ...
  const pickExcludedApp = async () => {
    try {
      ...
      if (!app) return;
      if (settings?.excludedApps.some((entry) => entry.bundleId === app.bundleId)) {
        notify("这个应用已在排除列表中");
        return;
      }
  ...
  return (
    <main className="settings-stage">
      <section className="settings-window">
        <header className="settings-header"><div><h1>EasyClipboard 设置</h1><p>所有内容只保存在这台 Mac</p></div><button className="icon-button" aria-label="关闭设置" onClick={close}><X /></button></header>
        <div className="settings-scroll">
          <section className="settings-card">
            <div className="setting-row"><div><strong>登录时启动</strong><span>开机后在菜单栏静默运行</span></div><Toggle label="登录时启动" checked={settings.launchAtLogin} onChange={(checked) => void save({ launchAtLogin: checked })} /></div>
          ...
          <section className="settings-section"><h2>快捷键与粘贴</h2><div className="settings-card">
            <div className="setting-row"><div><strong>打开剪贴板</strong><span>点击右侧后按下新的组合键</span></div>
              <button type="button" className={`shortcut-recorder ${recordingShortcut ? "recording" : ""}`} aria-pressed={recordingShortcut}
                onClick={() => setRecordingShortcut((current) => !current)}>{recordingShortcut ? "请按组合键…（Esc 取消）" : formatShortcut(settings.shortcut)}</button>
            </div>
            <div className="setting-row"><div><strong>辅助功能权限</strong><span>EasyClipboard 使用此权限切回目标应用并完成粘贴</span></div><span className={`permission-status ${permission?.accessibility ? "granted" : "required"}`}>{permission?.accessibility ? "已开启" : "必须开启"}</span></div>
            {!permission?.accessibility && <div className="permission-row"><Warning /><span>未开启时剪贴板主面板和历史记录将暂停使用。</span><button onClick={() => void requestAccessibility()}>开启辅助功能</button></div>}
          </div></section>
          ...
          <section className="settings-section"><div className="section-title-row"><h2>排除应用</h2><button className="mini-button" onClick={() => void pickExcludedApp()}><Plus />添加应用</button></div><div className="settings-card excluded-list">
            {settings.excludedApps.map((app) => <div className="excluded-app" key={app.bundleId}><span className="app-dot">{app.name.slice(0, 1)}</span><div><strong>{app.name}</strong><span>{app.bundleId}</span></div><button className="icon-button danger" aria-label={`移除 ${app.name}`} disabled={app.bundleId === "com.easyclipboard.desktop"} onClick={() => void save({ excludedApps: settings.excludedApps.filter((entry) => entry.bundleId !== app.bundleId) })}><X /></button></div>)}
          </div></section>
